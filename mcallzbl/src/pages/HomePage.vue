...
// 动态背景效果类型：每次随机选择
const effects: Array<'particles' | 'formulas' | 'hello-world'> = [
  'particles',
  'formulas',
  'hello-world',
]
const randomEffect = effects[Math.floor(Math.random() * effects.length)] as
  | 'particles'
  | 'formulas'
  | 'hello-world'
...
onMounted(() => {
  ...
  if (!(prefersReduced || isMobile)) {
    ...
    const ric = (window as Window & { requestIdleCallback?: RequestIdleCallbackFn })
      .requestIdleCallback
...
const friendItems: FriendItem[] = [
  {
    title: 'Jimmy的小屋',
    url: 'https://blog.jimmypowell.dev',
    description: 'jimmy的小屋 欢迎做客',
  },
]
</script>
